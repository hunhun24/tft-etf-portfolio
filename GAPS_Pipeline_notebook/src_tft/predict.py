...
실행: 
python src_tft_port_regime/predict.py \
...
def find_latest_checkpoint(ckpt_dir: Path) -> Path:
    ...
    # tft-v1 계열만 필터링 (29개짜리 구버전 제외)
    v1_ckpts = [p for p in ckpts if "v1" in p.name]
    if v1_ckpts:
        return v1_ckpts[-1]
    return ckpts[-1]
...
def run_inference(
    ...
    inf_df = inf_df.reset_index(drop=True)   # ← 이 한 줄 추가
...
def build_portfolio(pred_df: pd.DataFrame, pred_date: str) -> dict:
    ...
    # ── 비중 0 종목 제거 ──────────────────────────────────
    cons_w = cons_w[cons_w["weight"] > 0].reset_index(drop=True)
    agg_w  = agg_w[agg_w["weight"]  > 0].reset_index(drop=True)
    # ─────────────────────────────────────────────────────
