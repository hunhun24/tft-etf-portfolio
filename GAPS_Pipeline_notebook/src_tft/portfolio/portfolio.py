...
def apply_dbgaps_constraints(df, weights):
    ...
    ticker_caps = np.full(len(df), MAX_SINGLE_TICKER)  # 위험/안전 모두 20% 상한
    for _ in range(20):
        old = weights.copy()
...
def compute_weights(
    ...
    if strategy == "aggressive":
        ...
        # ── top_n개 초과 시 하위 종목 제거 ──
        # 비중 있는 종목만 top_n으로 제한
        nonzero_idx = np.where(weights > 1e-6)[0]
        if len(nonzero_idx) > top_n:
            # 비중 기준 상위 top_n만 유지
            keep_idx = nonzero_idx[np.argsort(weights[nonzero_idx])[::-1][:top_n]]
...
# ── 3.5 안전자산 최소 편입 보장 ──
# 위험자산만 top_n에 들어올 경우, 안전자산 강제 편입
    risk_count = (df["asset_class"] == "위험자산").sum()
    if risk_count == len(df):  # 전부 위험자산이면
        safe_candidates = signal_df[
         signal_df["asset_class"] == "안전자산"
        ].copy()
        safe_candidates["score"] = (
            safe_candidates["signal_sharpe"] * 0.7 +
            safe_candidates["signal_return"] * 0.3
        )
        # 안전자산 상위 3개 강제 편입
        safe_top = safe_candidates.nlargest(3, "score")
        # 위험자산 하위 3개 제거
        df = pd.concat([
            df.nlargest(top_n - 3, "score"),
            safe_top
        ], ignore_index=True)
    # ── 4. softmax 초기 비중 ──
    ...
    # ── 5.5 안전자산 부족 시 fallback 편입 (최대 top_n개 유지) ──
    risk_w = weights[df["asset_class"] == "위험자산"].sum()
    if risk_w > MAX_RISK_ASSET:
        safe_candidates = signal_df[
            (signal_df["asset_class"] == "안전자산") &
            (signal_df["signal_return"] >= 0)
        ].copy()
        safe_candidates["score"] = safe_candidates["signal_sharpe"] * 0.7 + safe_candidates["signal_return"] * 0.3
        safe_candidates = safe_candidates.sort_values("score", ascending=False).head(max(0, top_n - len(df)))  # 최대 3개만

        for _, row in safe_candidates.iterrows():
            ticker = str(row[GROUP_COL])
            if ticker in df[GROUP_COL].astype(str).values:
                continue
            new_row = row.copy()
            new_row["weight"] = 0.0
            df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
...
def portfolio_summary(weight_df: pd.DataFrame, strategy: StrategyType) -> dict:
    ...
    for k, v in asset_alloc.items():
        ...
        flag    = " [!]" if k == "위험자산" and v > MAX_RISK_ASSET + 1e-4 else " [OK]"
    ...
    for sub, w in sorted(sub_alloc.items(), key=lambda x: -x[1]):
        ...
        flag = " [!]" if w > cap + 1e-4 else " [OK]"
        print(f"    {sub:<22} {w*100:.1f}% / {cap*100:.0f}%{flag}")
    print(f"\n  Top5 종목")
    for h in summary["top_holdings"]:
        flag = " [!]" if h["weight"] > MAX_SINGLE_TICKER + 1e-4 else ""
...
def save_results(cons_df: pd.DataFrame, agg_df: pd.DataFrame, pred_date: str) -> None:
    cons_df = cons_df[cons_df["weight"] > 0].reset_index(drop=True)
    agg_df  = agg_df[agg_df["weight"]  > 0].reset_index(drop=True)
