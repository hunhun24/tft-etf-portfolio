"""
sarimax_backtest.py — SARIMAX top-10 포트폴리오 백테스트 및 TFT 비교

실행: python baseline/sarimax_backtest.py

입력:
  - baseline/sarimax_predictions.csv        (sarimax_baseline.py 실행 후 생성)
...
PRED_PATH = BASELINE_DIR / "sarimax_predictions.csv"
TFT_PATH  = (
    TFT_SRC / "outputs" / "portfolio" / "backtest_regime" / "backtest_returns.csv"
)
OUT_PATH  = BASELINE_DIR / "sarimax_backtest_comparison.csv"
...
def load_sarimax_preds() -> pd.DataFrame:
    if not PRED_PATH.exists():
        raise FileNotFoundError(
            f"SARIMAX 예측 파일 없음: {PRED_PATH}\n"
            "sarimax_baseline.py를 먼저 실행하세요."
        )
...
def main() -> None:
    print(SEP)
    print(f"SARIMAX top-{TOP_N} baseline vs TFT portfolio strategies")
    print(f"비교 성격: SARIMAX top-{TOP_N} baseline vs TFT portfolio strategies "
          f"(베이스라인 대비 부가가치 검증)")
    print(SEP)

    # ── 경로 ──
    print("\n[데이터 경로]")
    print(f"  SARIMAX 예측 : {PRED_PATH}")
    print(f"  TFT 결과     : {TFT_PATH}")

    # ── 로드 ──
    print("\n1. 데이터 로드")
    preds = load_sarimax_preds()
    tft   = load_tft_returns()
    print(f"  SARIMAX prediction date range : "
    ...
    # ── SARIMAX 백테스트 ──
    print(f"\n2. SARIMAX top-{TOP_N} + DB GAPS 백테스트")
    print(f"  SARIMAX 적용 제약: DB GAPS only (개별 20%, sub_asset_cap, 위험자산 70%)")
    rebal_dates = make_rebal_dates(preds)
    print(f"  SARIMAX 리밸런싱 날짜 수: {len(rebal_dates)}회  "
          f"({rebal_dates[0].date()} ~ {rebal_dates[-1].date()})")

    sarimax_result = xgb_backtest(preds, rebal_dates)
    sarimax_rets   = sarimax_result.set_index("date")["realized_return"].sort_index()

    # ── 공통 날짜 교집합 ──
    common_dates = sarimax_rets.index.intersection(tft.index)
    n_common     = len(common_dates)

    if n_common == 0:
        print(f"\n3. 공통 날짜 없음")
        print(f"  SARIMAX 기간: {sarimax_rets.index.min().date()} ~ {sarimax_rets.index.max().date()}")
    ...
    sarimax_c = sarimax_rets.loc[common_dates]
    ...
    sarimax_sample = sarimax_c.head(5).values
    rg_sample      = rg_c.head(5).values
    print(f"  target_5d 샘플 5개      : {[round(v, 6) for v in t5d_sample]}")
    print(f"  SARIMAX realized_return : {[round(v, 6) for v in sarimax_sample]}")
    print(f"  TFT regime_return       : {[round(v, 6) for v in rg_sample]}")

    # ── 성과 계산 ──
    rows = [
        analyze_performance(sarimax_c, "sarimax_top10_dbgaps"),
        analyze_performance(rg_c,      "tft_regime"),
        analyze_performance(eq_c,      "tft_equal_weight"),
    ]

    # ── 개별 출력 ──
    labels = [f"SARIMAX top-{TOP_N} (DB GAPS)", "TFT regime", "TFT equal_weight"]
